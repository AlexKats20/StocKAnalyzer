...
def analyze_stock(ticker, period, freq_str):
    ...
    interval_map = {'Daily': '1min', 'Weekly': '1week', 'Monthly': '1month'}  # Alpha Vantage intervals
    interval = interval_map.get(freq_str, '1min')
    ts = TimeSeries(key='LJTEHTJSP5O8XEOJ', output_format='pandas')
    
    st.write(f"⏳ Downloading data for {ticker} ({period}, {freq_str})")
    try:
        data, meta_data = ts.get_daily_adjusted(symbol=ticker, outputsize='full')
        if data.empty or len(data) < 14:
            st.warning(f"Insufficient data ({len(data)} rows) for {ticker}. Try a longer period or different frequency.")
            raise ValueError("Insufficient data returned for the specified period and frequency.")
        
        df = data[['1. open', '2. high', '3. low', '4. close', '6. volume']].rename(columns={
        ...
        # Plot window
        df_plot = df.tail(250 if interval_map.get(freq_str, '1min') == '1min' else 52 if interval_map.get(freq_str) == '1week' else 24) if actual_period != 'max' else df
        ...
        df_occ = df.tail(250 if interval_map.get(freq_str, '1min') == '1min' else 52 if interval_map.get(freq_str) == '1week' else 24) if actual_period != 'max' else df
...
st.info("For short periods (e.g., 1mo) with Monthly frequency, select a longer period (e.g., 3mo or 6mo) or Daily/Weekly frequency to ensure sufficient data. Requires a free Alpha Vantage API key (sign up at https://www.alphavantage.co/support/#api-key).")
